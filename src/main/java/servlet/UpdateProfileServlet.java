 ...
 * Handles profile update requests from the customer profile page
 */
@MultipartConfig
public class UpdateProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (action) {
            case "update_info":
                updateUserInfo(request, response, user, session);
                break;
            case "change_password":
                changePassword(request, response, user);
                break;
            case "update_picture":
                updateProfilePicture(request, response, user, session);
                break;
     ...
     */
    private void updateUserInfo(HttpServletRequest request, HttpServletResponse response, User user, HttpSession session)
    ...
    private void changePassword(HttpServletRequest request, HttpServletResponse response, User user)
        ...
        if (currentPassword == null || newPassword == null || confirmPassword == null ||
            ...
            if (user.isAdmin()) {
                response.sendRedirect(request.getContextPath() + "/admin/profile.jsp?passwordError=All+fields+are+required");
            } else {
                response.sendRedirect(request.getContextPath() + "/customer/profile.jsp?passwordError=All+fields+are+required");
            }
            return;
        }
     ...
     */
    private void updateProfilePicture(HttpServletRequest request, HttpServletResponse response, User user, HttpSession session)
            throws ServletException, IOException {
        try {
            // Handle file upload
            ...
            e.printStackTrace();
            if (user.isAdmin()) {
                response.sendRedirect(request.getContextPath() + "/admin/profile.jsp?error=Error+uploading+profile+image:+" + e.getMessage());
            } else {
                response.sendRedirect(request.getContextPath() + "/customer/profile.jsp?error=Error+uploading+profile+image:+" + e.getMessage());
            }
        }
    }
     ...
     */
    private void deactivateAccount(HttpServletRequest request, HttpServletResponse response, User user, HttpSession session)
            throws ServletException, IOException {
        // Get form data
    ...
    /**
     * Utility method to get the submitted file name from a Part
     */
