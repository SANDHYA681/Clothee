...
public class OrderServlet extends HttpServlet {
    ...
    /**
     * Cancel an order
     * Note: Customers can no longer cancel orders as they have already paid
     */
    private void cancelOrder(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!user.isAdmin()) {
            // Customers cannot cancel orders as they have already paid
            response.sendRedirect(request.getContextPath() + "/OrderServlet?action=viewOrders&error=Customers+cannot+cancel+orders+as+payment+has+been+made");
            return;
        }
